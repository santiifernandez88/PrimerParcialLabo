...
void ObtenerString(char mensaje[],char input[])
{
    printf("%s",mensaje);
    fflush(stdin);
    scanf("%[^\n]s", input);
...
float getValidFloat(char mensajePedido[],char mensajeError[])
{
    char auxStr[256];
    int flag=0;
    float auxFloat;
    do
    {
        flag=getStringNumerosFlotantes(mensajePedido,auxStr);
        if(flag!=1)
        {
            printf ("%s\n",mensajeError);
            continue;
...
int NumeroRepetido(int array[], int i, int numAleatorio, int tam)
{
	int retorno = 1;
